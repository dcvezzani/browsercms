...
# includes ./bin from project's root (bundle install --binstub)
# this allows for development on an unpacked gem
...
After do
  # when rake is called, the ./bin/bcms* files are modified from what we want
  # (from rake db:install?)
  # once these files are changed, the tests start failing again
  # make sure these are reset for the next test
  `git co -- bin/bcms bin/bcms-upgrade bin/browsercms`
end
